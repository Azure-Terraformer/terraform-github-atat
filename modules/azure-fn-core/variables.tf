...
variable "os_type" {
  type        = string
  description = <<DESCRIPTION
  The operating system type for Azure Function Service Plan that will be provisioned."
DESCRIPTION
}
variable "sku_name" {
  type        = string
  description = <<DESCRIPTION
  The SKU for the Azure Function Service Plan.
DESCRIPTION
}
variable "retention_in_days" {
  type        = number
  description = <<DESCRIPTION
  The number of days to retain logs before automatic deletion from the Azure Log Analytics Workspace in the Azure Functions Core Environments.
DESCRIPTION
}
